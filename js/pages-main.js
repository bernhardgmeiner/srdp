...
PAGES.home = {
  ...
  render() {
    ...
    const quizzes = ['essay', 'article', 'report', 'blog', 'email', 'final'].filter(id => prog[id] && prog[id].quiz).length;
    ...
    return '<div class="page">' +
      '<div class="page-head" style="padding-bottom:56px"><div class="inner" style="max-width:1100px">' +
        '<div class="eyebrow"><span>AHS Schriftliche Reifeprüfung · English B2 (<a href="https://www.bernhardgmeiner.com" target="_blank" rel="noopener">Bernhard Gmeiner</a>)</span></div>' +
        '<h1 class="title" style="font-size:clamp(2.3rem,5.5vw,4.4rem);max-width:880px">Don&rsquo;t panic, it&rsquo;s just the Matura.</h1>' +
        '<p class="lead">Everything you need for the two writing tasks of the English Matura. Start with the overview, or jump straight to the text type you are practising this week.</p>' +'<p lang="de" style="font-size:.9375rem;color:var(--text-muted);max-width:620px;margin:-14px 0 26px;line-height:1.5">Diese Seite hilft bei der Vorbereitung auf die schriftliche Englisch-Matura (AHS, B2). Alle Lerninhalte sind auf Englisch.</p>' +
        '<div style="display:flex;gap:1px;flex-wrap:wrap">' +
          ...
          '<a class="btn btn-ghost" href="#essay" style="min-width:170px">Jump to the essay <span>→</span></a>' +
        '</div>' +
      '</div></div>' +
      '<div class="wrap" style="max-width:1100px">' +
        (M.homeHint ? M.homeHint(visited > 0 ? 'Your progress: ' + visited + '/' + trackable.length + ' sections · ' + quizzes + '/6 quizzes' : '', next) : '') +
        '<div class="gap-s"></div>' +
        ...
        '<div class="grid g-auto-240">' +
          [
            ...
            ['essay', '02', 'Pick a text type', 'Each of the five types has a guide, a model text, phrases and a quiz.'],
        ...
        '<div class="grid g-auto-150">' +
          [['120 min', 'total writing time'], ['2 tasks', '~400 + ~250 words'], ['4 × 0–10', 'equally weighted criteria'], ['±10%', 'word count tolerance']].map(s =>
            '<div class="card"><div class="stat-v">' + s[0] + '</div><div class="stat-l">' + s[1] + '</div></div>').join('') +
        '</div>' +
        '<div class="gap"></div>' +
        sectionLabel('Five text types') +
        '<div class="grid g-auto-280">' +
          SRDP.textTypes.map(t => {
        ...
        '<div class="grid g-auto-220">' +
          [
            ['phrasebank', 'Phrase bank', '160+ exam-ready phrases, one-click copy'],
            ...
            ['taskbank', 'Task bank', '24 Matura-style tasks for every text type'],
  ...
  render() {
    ...
    return '<div class="page">' +
      ...
      '<div class="wrap"><div class="gap-s"></div>' +
        ...
        '<p style="font-size:1.1rem;color:var(--text-secondary);line-height:1.6;margin-bottom:22px;max-width:720px">You have <strong style="color:var(--text)">120 minutes</strong> for <strong style="color:var(--text)">two tasks</strong>: one longer (~400 words, at B2 usually the essay) and one shorter (~250 words). Writing is one of four equally weighted sections of the written exam, alongside Reading, Listening and Language in Use.</p>' +
        '<div class="tip">Time split that works: ~65 min for the 400-word task (incl. 5–10 min planning) · ~40 min for the 250-word task · the rest for proofreading both. Write your finishing times on the task sheet before you start.</div>' +
        '<div class="gap-s"></div>' +
        ...
        '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px">' +
          '<div class="tip">Word count tolerance: ±10%. That is 360–440 words for 400-word tasks and 225–275 for 250-word tasks. If you are further off than that, Task Achievement is reduced by one band.</div>' +
          '<div class="tip">Every task gives three content points. Address all three — each in its own body paragraph.</div>' +
        '</div>' +
        ...
        '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Mistake</th><th>What it costs</th><th>The fix</th></tr></thead><tbody>' +
          [
...
SRDP.textTypes.forEach(t => {
  PAGES[t.id] = {
    ...
    wire() {
      ...
      if (tab === 'quiz') {
        if (!M.quizStates[t.id] || M.quizStates[t.id]._stale) M.initQuiz(t.id, SRDP.quizzes[t.id], (score, total) => { if (score >= Math.ceil(total * 0.6)) M.markQuiz(t.id); });
        const host = M.$('[data-quiz-host="' + t.id + '"]');
        if (host) host.innerHTML = M.quizHTML(t.id);
      }
      if (tab === 'dnd') {
        M.initDnd(t.id, SRDP.dndSets[t.id]);
        M.repaintDnd(t.id);
      }
    },
  };
