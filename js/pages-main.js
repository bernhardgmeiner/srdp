...
const SECOND_MODELS = {
  essay: { title:'Should smartphones be banned in schools?', meta:'~400 words · formal', tip:'This second example has no colour labels on purpose. Try to spot the thesis in the introduction, the topic sentence of each body paragraph, the counter-argument and the forward-looking conclusion yourself.', paras:[{ t:'Few objects divide a staffroom as quickly as the smartphone. Some teachers would happily collect every device at the school gate; others hand out tablets and expect pupils to look things up in seconds. My own view sits between the two. A blanket ban is a mistake: phones cause genuine problems in class, but pretending they do not exist solves nothing, and the realistic option is to control when and how they are used.', mono:false },{ t:'The case for banning phones is easy to understand. A single buzzing notification can pull a whole row of students out of a lesson, and anyone who has tried to explain the past perfect to a class hiding TikTok under the desk knows how quickly attention drains away. Cheating has become simpler, too: an answer is only a quick search away. Several schools that locked phones in pouches during the day have reported calmer corridors and livelier conversations at break. These are not small gains, and they deserve to be taken seriously.', mono:false },{ t:'Even so, the device that causes the trouble is also a genuinely useful tool. In my own biology class we photograph experiments, scan a code to open a quiz, and check unfamiliar words in seconds. A student who has forgotten a deadline can look it up; one who finds reading hard can have a text read aloud. Banning the phone outright throws all of this away, and it teaches nothing about self-control. Teenagers who never manage their phones at school simply postpone that struggle until later.', mono:false },{ t:'Supporters of a full ban often reply that teenagers cannot be trusted to regulate themselves, and there is some truth in that. Willpower rarely wins against a well-designed app. The better answer, though, is to build in structure. Phones can stay switched off and out of sight by default, and come out only for a clear purpose when a teacher asks for them. Rules that students help to write are far harder to resent than ones simply imposed from above.', mono:false },{ t:'On balance, I remain convinced that banning smartphones treats a symptom while ignoring the cause. The pupils in front of us will carry these devices into every job and every waiting room for the rest of their lives. A school that shows them how to switch off, focus, and then use the tool on purpose gives them something a locked pouch never can.', mono:false }] },
