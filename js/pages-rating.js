...
const EXAMPLES = [
  /* 1 · ESSAY (AHS) — competent, mid-range, position a touch soft */
  {
    ...
    ratings: {
      TA: { band: 6, points: [
        'The purpose — persuade the reader — is met, and a valid position is there ("a small job can teach a great deal, as long as it does not take over"). The problem is placement, not the stance: it lands in the last clause of a long sentence, where a short, assertive thesis would hit harder.',
      ...
      LR: { band: 7, points: [
        ...
        'A range of structures — when-clauses, conditionals, relative clauses ("A teenager who works ...") — with only occasional repetition.',
  ...
  /* 3 · REPORT (all) — the strong anchor */
  {
    ...
    ratings: {
      TA: { band: 8, points: [
        ...
        'All three prompts are developed with actual figures ("74%", "almost half"), and the sections carry clear, meaningful headings — correctly not numbered.',
      ...
      LR: { band: 7, points: [
        ...
        'Some complex structures — passive ("should be spread"), relative clause ("which forces students to choose").',
  ...
  /* 5 · EMAIL (all) — does its job, held down by accuracy + register slips */
  {
    ...
    ratings: {
      ...
      LA: { band: 6, points: [
        ...
        'The clear slips are "any informations" (uncountable) and the missing question mark on "do the students get a certificate at the end" — a handful, not a pattern.',
...
function critBlock(ex, c) {
  ...
  const isTA = c.id === 'TA';
  return '<div class="ex-crit" data-awarded="' + r.band + '">' +
  ...
  return '<div class="ex-card" id="ex-' + ex.id + '" style="--exc:' + col + '">' +
    ...
    '<div class="acc ex-holi"><div class="acc-item"><button class="acc-head" data-action="acc" aria-expanded="false"><span class="pm">+</span><span>Overall verdict</span><span class="sub">reveal after you grade</span></button>' +
      '<div class="acc-body"><p class="ex-holi-p">' + esc(ex.holistic) + '</p>' +
        '<div class="ex-total">Four criteria, added up: <b>' + tot + ' / 40</b></div></div></div></div>' +
  '</div>';
}
  ...
  render: function () {
    ...
    const process =
      '<div class="ex-process"><ol>' +
        ...
        '<li><b>They rate one criterion at a time.</b> Each of the four is judged on its own, often in its own reading pass. The same text can sit at 8 for Range and 6 for Accuracy — that is normal, not a contradiction.</li>' +
    ...
    const approp =
      '<p class="ex-lead">Two criteria can judge the very same word. <strong style="color:var(--text)">Coherence &amp; Cohesion</strong> asks whether a linking word <em>fits</em>. <strong style="color:var(--text)">Accuracy</strong> asks whether it is <em>correct English</em>. A word can pass one and fail the other — and it is counted under both, once as a plus, once as a minus.</p>' +
      '<div class="ex-approp">' +
      ...
      '</div>' +
      '<p style="font-size:.9rem;color:var(--text-secondary);margin-top:10px;max-width:720px">So reach for linking words freely — they lift your Coherence score. Then, when you proofread, check their <em>form</em>, because the same words are counted again under Accuracy.</p>';

    return pageHead(
      'Basics',
      'Grade like an examiner',
      'Reading about the four criteria is one thing. Watching them land on a real text is another. Here is how a rater actually works — and a set of texts for you to grade yourself before the panel shows its hand.'
    ) +
